package com.shinhan.formyegg.global.util;
...
public class JwtTokenProvider {
    ...
    // 토큰 생성
    public String createToken(long memberNo, int incomeRank){
        Claims claims = Jwts.claims().setSubject(String.valueOf(memberNo));
        claims.put("income", incomeRank);
        Date now = new Date();
        return Jwts.builder()
                .setClaims(claims)
    ...
    public Authentication getAuthentication(String token){
        ...
        Collection<? extends GrantedAuthority> authorities =
                Arrays.stream(new String[] {claims.get("income").toString()})
    ...
    public boolean validateToken(String token) {
        try {
            ...
            throw new NonValidJwtTokenException(ErrorCode.NON_VALID_TOKEN);
        }
    }
