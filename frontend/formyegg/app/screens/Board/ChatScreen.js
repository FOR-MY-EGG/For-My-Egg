...
const ChatScreen = ({navigation, route}) => {
  ...
  let socket = useMemo(() => new SockJs('http://ec2-3-39-138-177.ap-northeast-2.compute.amazonaws.com:8080/ws/chat'), []);
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      http
        .get(`chat/${route.params.affiliation}`)
        .then(res => {
          setMessage(res.data);
        })
        .catch(err => {
  ...
  const onConnected = () => {
    stompClient.subscribe(
      `/topic/room.${route.params.affiliation}`,
      onMessageReceived,
      {
        id: 'curRoom',
        'auto-delete': true,
...
const MyMessage = ({nickname, createdDate, content}) => (
  <View style={styles.myMessage}>
    <View>
      ...
      <Text style={[styles.myText, styles.date]}>
        {new String(createdDate).slice(11)}
      </Text>
    </View>
    <Image
      style={styles.profile}
      source={{
        uri: 'https://formyegg-bucket.s3.ap-northeast-2.amazonaws.com/user.png',
      }}
    />
  </View>
);
...
const OtherMessage = ({nickname, createdDate, content}) => (
  <View style={styles.otherMessage}>
    ...
    <View>
      ...
      <Text style={[styles.otherText, styles.date]}>
        {new String(createdDate).slice(11)}
      </Text>
    </View>
  </View>
  ...
  <View
    ...
    <TouchableOpacity onPress={onSend}>
      <Image
        style={styles.button}
        source={{
          uri: 'https://formyegg-bucket.s3.ap-northeast-2.amazonaws.com/send.png',
        }}></Image>
    </TouchableOpacity>
  </View>
);
